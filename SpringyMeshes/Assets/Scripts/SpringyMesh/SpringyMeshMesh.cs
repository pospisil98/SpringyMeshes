...
public class SpringyMeshMesh : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        float totalArea = faces.Sum(f => f.area);
        for (int i = 0; i < vertices.Count; i++) {
            float vertexMass = 0.0f;
            HashSet<int> n = neighbourFaces[i];
            foreach (int neiFaceIndex in n) {
                vertexMass += (faces[neiFaceIndex].GetAngleByVertex(vertices[i]) / 180.0f) * (faces[neiFaceIndex].area / totalArea) * mass;
            }
